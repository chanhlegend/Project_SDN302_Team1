      ...
      if (payment_method === "2") {
        for (const sellerId in groupedProducts) {
          ...
          const newOrder = new Order({
            ...
            totalPrice:
              sellerProducts.reduce((acc, p) => acc + p.price, 0) + 7000,
            isPaid: false,
          });
    ...
    try {
      ...
      if (secureHash === signed) {
        ...
        if (rspCode === "00") {
          ...
          // Khởi tạo groupedProducts ngay từ đầu
          let groupedProducts = {};

          // Lấy sản phẩm từ database dựa trên trạng thái và người bán
          ...
          for (const sellerId in groupedProducts) {
            ...
            const totalProductPrice = sellerProducts.reduce(
              (acc, p) => acc + p.price,
              0
            );
            const totalPrice = totalProductPrice + 7000; // Thêm phí vận chuyển
